  ...
  return (
    <div>
      <Flex mb={"16px"} justify='space-between'>
        ...
        <Button color="teal" to='/levels/create' component={ Link }> Cadastrar Novo Nível </Button>
      </Flex>
      {isSuccess && (
        <Notification
          ...
          {levels.map((level) => {
            return (
              <tr key={level.id}>
                ...
                <td>
                  <ActionIcon color="blue" variant="outline" to={`/levels/edit/${level.id}`} component={ Link }>
                    <IconPencil size="1.125rem" />
                  </ActionIcon>
                </td>
                <td>
                  <ActionIcon color="red" variant="outline" onClick={() => deleveLevel(level.id)}>
